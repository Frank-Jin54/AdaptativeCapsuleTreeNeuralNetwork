...
class Tree():
    ...
    def __init__(self,
        ...
        # case (1): splitting
        if split:
            self.child_left = tf.keras.Sequential()
            self.child_left.add(child_left["transform"])
            self.child_left.add(child_left["classifier"])
            self.child_left.add(child_left["router"])
            self.child_right = tf.keras.Sequential()
            self.child_right.add(child_right["transform"])
            self.child_right.add(child_right["classifier"])
            self.child_right.add(child_right["router"])
        
        # case (2): making deeper
        if extend:
            self.child_extension = tf.keras.Sequential()
            self.child_extension.add(child_extension["transform"],
            )
            self.child_extension.add(child_extension["classifier"],
            )
            self.child_extension.add(child_extension["router"],
            )
...
class RouterGAPwithConv_TwoFClayers():
    """ Routing function:
    Conv2D + GAP + fc1 + fc2 
    """
    def __init__(self, input_nc,  input_width, input_height,
...
class MLP_AlexNet():
    ...
    def __init__(self, input_nc, input_width, input_height, dropout_prob=0.0, **kwargs):
        ...
        self.dropout_prob = dropout_prob
        ngf = input_nc * input_width * input_height
        self.fc1 = tf.keras.layers(ngf, 128)
        self.fc2 = tf.keras.layers(128, 10)
